...
module.exports = message => {
  ...
  if (!client.commands.has(command)) {
    if (client.aliases.has(command)) {
      ...
      message.channel.send(`Komutlarımda **${command}** adında bir komut bulamadım!`)
    }
  }
...
client.on("guildMemberAdd", async member => {
  ...
  client.channels.cache.get(hgmesajı).send(`:tada: Sunucumuza hoşgeldin ${member} \n\n Hesabın ${moment(member.user.createdAt).format("**DD MMMM YYYY hh:mm:ss**") } tarihinde oluşturulmuş\n\nSunucu Kurallarımızı Okumayı Unutma. Sunucu içerinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek\n\nSeninle beraber ${member.guild.memberCount} kişi olduk ! Tagımızı alarak bizlere destek olabilirsin ! Kayıt olmak için c!kayıt isim yaş İyi eğlenceler.`);
});
...
client.on("guildMemberAdd", async member => {
  ...
  client.channels.cache.get(yetkilihgmesajı).send(`:tada: Sunucumuza hoşgeldin ${member} \n\n Hesabın ${moment(member.user.createdAt).format("**DD MMMM YYYY hh:mm:ss**") } tarihinde oluşturulmuş\n\nSunucu Kurallarımızı Okumayı Unutma. Sunucu içerinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek\n\nSeninle beraber ${member.guild.memberCount} kişi olduk ! Tagımızı alarak bizlere destek olabilirsin ! Kayıt olmak için teyit odalarına girip ses teyit vermen gerekiyor yetkililerimiz senile ilgilenecektir ! İyi eğlenceler.'`);
});
...
client.on("guildMemberAdd", async member => {
  ...
  const skanal31 = member.guild.channels.find("name", skanal9);
  if (!skanal31) return;
  skanal31.send(`:loudspeaker\`${ member.user.tag }\` Adlı Kullanıcı Sunucuya Katıldı. \`${sayac}\` Kullanıcı Olmaya \`${sayac - member.guild.members.size}\` Kullanıcı Kaldı ! <a:hg:706589034226647041>  ` );
});
...
client.on("guildMemberRemove", async member => {
  ...
  const skanal31 = member.guild.channels.find("name", skanal9);
  if (!skanal31) return;
  skanal31.send(`:loudspeaker\`${  member.user.tag }\`Adlı Kullanıcı Sunucudan Ayrıldı. \`${sayac}\` Kullanıcı Olmaya \`${sayac - member.guild.members.size}\` Kullanıcı Kaldı ! <a:by:706589001091383347> `);
});
...
member.roles.add(rol)
  ...
  member.guild.channels.cache.get(kanal).send(`**${member.user.username}** Sunucuya katıldı. Başarıyla, \`${role}\` rolü verildi.`)
...
if(!hedef) return;
client.channels.cache.get(kanal).send(`${member} sunucuya katıldı. **${sayac}** kişi olmamıza, **${hedef - member.guild.memberCount}** kişi kaldı!`)
...
var hedef = qdb.fetch(`sayachedef_${member.guild.id}`)
if(!hedef) return;
client.channels.cache.get(kanal).send(`${member.user.tag} sunucudan ayrıldı. **${sayac}** kişi olmamıza, **${hedef - member.guild.memberCount}** kişi kaldı!`)
})
